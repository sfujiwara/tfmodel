...
class Vgg16:
    ...
    @deprecated("2017-07-17", "Vgg16 class is deprecated. Please use build_vgg16_graph function.")
    def __init__(self, img_tensor, reuse=False, trainable=True, include_top=True):
        self.saver = None
        self.logits = build_vgg16_graph(
    ...
    def restore_pretrained_variables(self, session):
        # Create saver
        if not self.saver:
            self.saver = tf.train.Saver(var_list=tf.get_collection(VGG16_GRAPH_KEY))
        # Download weights
